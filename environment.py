...
class Environment:
    def __init__(self, urdf_path, timestep, q0l, circle):
        ...
        print(base_path)
        self.left_arm = self._client.loadURDF(
            os.path.join(base_path, urdf_path),
        ...
        for i in range(3):
            v = [0.0] * 3
            v[i] = 1.0
            self._client.addUserDebugLine([0.0, 0.0, 0.0], v, v)
        ...
        idx = pybullet.createMultiBody(
            baseVisualShapeIndex=sphere, basePosition=[circle["x"], circle["y"], circle["z"]]
        )
    ...
    def set_control(self, u):
        ...
        for i in range(self.joint_num):
            self._client.setJointMotorControl2(
                self.left_arm, self.joint_idx[i], pybullet.TORQUE_CONTROL, force=u[i]
            )
    ...
    def reset_joints_state(self, q0l, qdot0l):
        ...
        for i in range(self.joint_num):
            self._client.setJointMotorControl2(
                self.left_arm, self.joint_idx[i], pybullet.POSITION_CONTROL, force=0
            )
        for i in range(self.joint_num):
            self._client.resetJointState(
                self.left_arm, self.joint_idx[i], q0l[i], qdot0l[i]
            )

    def acc_to_ctrl_action(self, ddq):
        # ddq = np.clip(ddq, -10., 10.)
        q, dq = self.get_state()
        tau = self._client.calculateInverseDynamics(
            self.left_arm, q.tolist(), dq.tolist(), ddq.tolist()
        )
        return tau
    ...
    def get_state(self):
        ...
        for i in range(self.joint_num):
            q[i], qdot[i], _, _ = self._client.getJointState(
                self.left_arm, self.joint_idx[i]
            )
        return q, qdot
